...
exports.CompetitiveEdgeStrategy = void 0;
const trading_utils_1 = require("../utils/trading-utils");
class CompetitiveEdgeStrategy {
    /**
     * Advanced momentum strategy with volume confirmation
     */
    static generateMomentumSignals(marketData) {
        const signals = [];
     ...
     */
    static generateMeanReversionSignals(marketData) {
        const signals = [];
        for (const [symbol, data] of Object.entries(marketData)) {
            if (symbol === 'USDC')
                continue;
     ...
     */
    static detectArbitrageOpportunities(marketData) {
        const signals = [];
     ...
     */
    static generateRebalanceSignals(currentPortfolio, targetAllocation, marketData) {
        const signals = [];
        const totalValue = Object.entries(currentPortfolio).reduce((sum, [symbol, amount]) => sum + (amount * (marketData[symbol]?.price || 0)), 0);
        if (totalValue === 0)
            return signals;
        const drift = trading_utils_1.TradingUtils.calculateDrift(Object.fromEntries(Object.entries(currentPortfolio).map(([symbol, amount]) => [
            symbol,
     ...
     */
    static generateAllSignals(marketData, currentPortfolio, targetAllocation) {
...
exports.CompetitiveEdgeStrategy = CompetitiveEdgeStrategy;
//# sourceMappingURL=competitive-edge.js.map